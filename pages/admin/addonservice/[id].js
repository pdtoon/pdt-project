...
const Addonservice = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:4000/addonservice/"+Router.query.id).then((res) => {
    ...
    const onSubmit = () => {
        console.log(name)
        axios.put("http://localhost:4000/addonservice/"+Router.query.id, {
            name: name,
            price: price,
        }).then((res) => Router.replace("/admin/addonservice/addonservice"))
    }
